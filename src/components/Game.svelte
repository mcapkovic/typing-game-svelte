<script>
  ...
  function textValidation(value) {
    ...
    if (newPossition < possition) {
      letters[newPossition + 1].state = DEFAULT_STATE;
      firstErrorIndex =
        firstErrorIndex === newPossition + 1
          ? Number.MAX_VALUE
          : firstErrorIndex;
      ...
      if (value === " ") correctWords = correctWords + 1;
...
<div class="game">
  <div class="game__text">
    {#each letters as letter, index}
      <span
        class={`game__text__letter game__text__letter--${
          index === possition + 1 ? CURSOR_STATE : letter.state
        }`}>{letter.value}</span
      >
    {/each}
  </div>
